...
// **UPGRADED with a reliable spaCy entity extractor AND a highly-detailed BRD generation prompt.**
// **FIXED with an in-memory cache to handle concurrent requests efficiently and robust Mermaid code cleaning.**
// **FIXED AGAIN with a stricter flowchart prompt to prevent AI from generating invalid syntax.**
// **FINAL FIX: Implemented promise-based caching to resolve the race condition from parallel requests.**
// **NEW FEATURE: Added a query parameter (?noCache=true) to bypass caching for development and testing.**
// **DEFINITIVE FIX: Added a final cleaning step to the AI's flowchart output to remove invalid separator lines.**
...
// ===================================================================================
// --- Caching Implementation (FIXED FOR RACE CONDITION & ADDED BYPASS) ---
// ===================================================================================
...
async function getOrGenerateArtifacts(file, options = {}) {
    const { useCache = true } = options;
    const cacheKey = getCacheKey(file.buffer);
    const reqId = uuidv4().slice(0, 8);

    if (useCache && processCache.has(cacheKey)) {
        console.log(`[${reqId}] [CACHE HIT] Found existing process in cache for key: ${cacheKey.substring(0,10)}...`);
        return processCache.get(cacheKey);
    }
    
    if (!useCache) {
         console.log(`[${reqId}] [CACHE DISABLED] Bypassing cache for key: ${cacheKey.substring(0,10)}...`);
    } else {
        console.log(`[${reqId}] [CACHE MISS] Starting new generation process for key: ${cacheKey.substring(0,10)}...`);
    }
    
    const generationPromise = (async () => {
        try {
            const fileContent = await getFileContent(file);
            const { anonymizedText, mapping } = await anonymizeText(fileContent);
            const generatedBRD = await generateBRD(anonymizedText);
            
            return { fileContent, anonymizedText, mapping, generatedBRD };
        } catch (err) {
            if (useCache) {
    ...
    if (useCache) {
        processCache.set(cacheKey, generationPromise);

        generationPromise.then(() => {
            setTimeout(() => {
                processCache.delete(cacheKey);
                console.log(`[CACHE] Cleared cache for key: ${cacheKey.substring(0,10)}...`);
            }, 300000);
        }).catch(() => {
            // Error handling inside the promise already removes the key.
        });
    }
...
---
HERE IS A GOLD-STANDARD EXAMPLE OF A COMPLETED BRD.
ANALYZE THIS EXAMPLE TO UNDERSTAND THE REQUIRED STYLE, TONE, AND LEVEL OF DETAIL.
THEN, APPLY THE SAME LEVEL OF QUALITY AND SYNTHESIS TO THE NEW TRANSCRIPT YOU ARE GIVEN.
ANALYZE THIS EXAMPLE TO UNDERSTAND THE REQUIRED STYLE, TONE, AND LEVEL OF DETAIL.

# Dish Network / EchoStar – PO Accrual Automation Business Requirements Document (BRD)

## 1. Document Control
| Version | Author | Date | Status | Reviewers |
|---|---|---|---|---|
| 0.1 (Draft) | AI Business Analyst | 18-Jul-2025 | Draft – For Review | "Trisha (IT Expense), Kartik Subbaraman (Consultant), Payal Tiwana (Program Lead)" |

## 2. Executive Summary
Dish Network’s IT Expense group currently performs highly manual, error-prone month-end activities to accrue professional-services costs for staff-augmentation and consulting engagements. Data is scattered across ServiceNow, Oracle E-Business Suite, Orbitz BI, OneStream and shared e-mail inboxes. The proposed solution introduces Gen-AI agents and automations that will extract data, calculate accurate accruals, generate journal-entry workbooks, create release POs, validate postings and communicate with vendors – reducing close effort and improving accuracy.

## 3. Business Objectives
* Reduce month-end accrual cycle time from 5 days to <1 day.
* Eliminate >90% of manual spreadsheet manipulation and data reconciliation tasks.
* Provide executives with real-time visibility of PO commitment vs. invoice vs. accrual exposure.
* Improve auditability and SOX compliance by enforcing rule-based, system-generated journals.

## 4. Project Scope
**In Scope**
* Professional-services (staff-aug & consulting) requisitions raised through ServiceNow.
* Oracle blanket-purchase agreements (BPAs) & standard POs related to labour.
* Invoice intake via IT Expense & AP mailboxes and OCR pipeline.
* Accrual calculation, JE generation & posting validation.
* Automated vendor outreach for un-billed services.

**Out of Scope**
* Capital-asset depreciation, payroll and inventory accruals.
* Migration of EchoStar/Hughes processes until harmonised platform is selected.

## 5. Stakeholder Analysis
| Name | Role / Dept. | Interest | Contact |
|---|---|---|---|
| Trisha | Manager – IT Expense | Process owner & primary SME | TBD |
| Kartik Subbaraman | External Consultant | Solution architect & analyst | TBD |
| Payal Tiwana | Program Lead | Executive sponsor & delivery oversight | TBD |
| Accounting Analyst (TBD) | IT Accounting | SME – accrual preparation | TBD |
| COO | Executive Approver | Approves POs > $100k | TBD |
| "Vendors (e.g., Tech Mahindra)" | Service Providers | Invoice submission | Multiple |

## 6. Current State Overview
1. Requesters raise IT purchase requisitions in **ServiceNow** and attach statements of work (SOWs).
2. Requisitions route for executive approval; upon approval an Oracle BPA/PO is created manually.
3. Vendors e-mail invoices to multiple inboxes; AP OCR uploads some invoices, triggering holds when no PO release exists.
4. IT Expense team manually creates BPA releases, reconciles ServiceNow, Orbitz and Oracle data, and compiles accrual spreadsheets.
5. Spreadsheet journal entries are e-mailed to disparate accounting teams, who key them into Oracle; IT Expense spot-checks postings in OneStream.
6. Limited reporting exists; executives lack visibility to PO burn-down or outstanding accrual exposure.

## 7. Future State Vision
* AI agents automatically extract requisition, SOW and invoice data; data lake used for unified view.
* Automated accrual engine calculates required accruals by cost-centre, project and account based on rules & SOW milestones.
* JE workbooks auto-generated, routed for approval and system-posted via Oracle API.
* Vendor statement requests auto-generated; dashboard provides real-time PO / invoice / accrual analytics.

## 8. Functional Requirements
| ID | Description | Priority | Acceptance Criteria | Source |
|---|---|---|---|---|
| FR-1 | Ingest purchase-requisition data from ServiceNow including BPA/PO # and attached SOW. | Must | System successfully queries ServiceNow API and stores requisition JSON daily. | Transcript 0:53-3:42 |
| FR-2 | Ingest Oracle BPA and release transactions to identify matched / unmatched invoices. | Must | Daily ETL loads BPA, release and invoice tables into data lake. | 9:22-11:35 |
| FR-3 | Calculate monthly accruals for staff-augmentation based on ceiling ÷ service-term where invoice missing. | Should | Accrual journal equals (ceiling ÷ months) minus paid invoices for period. | 12:12-13:54 |
| FR-4 | Parse attached SOW to derive milestone-based accrual schedule. | Could | AI extracts milestone dates & amounts with >90% accuracy in sample SOWs. | 22:07-23:40 |
| FR-5 | Generate accrual JE workbook and route to accounting for approval. | Must | Workbook conforms to Accounting template and is delivered <60 min after data cut-off. | 33:26-34:27 |
| FR-6 | Auto-post approved JEs to Oracle via API and confirm in OneStream. | Should | Postings succeed without manual keying; OneStream reflects balances within 30 min. | 34:27-36:22 |
| FR-7 | Send e-mail to vendors requesting un-billed statement each period. | Could | ≥85% vendor response rate by period-end. | 31:19-32:52 |
| FR-8 | Auto-create Oracle BPA releases for invoices ≤$50K where hold exists. | Should | Releases created and holds released within 30 min of invoice OCR import. | 45:42-46:33 |
| FR-9 | Classify incoming invoices from multiple inboxes and route correctly. | Must | ≥98% invoice-PO match rate; mis-routed invoices <2%. | 42:09-45:29 |

## 9. Non-Functional Requirements
| ID | Description | Metric / Target | Priority |
|---|---|---|---|
| NFR-1 | Performance – month-end accrual run | Complete <1 hr for 100 SOWs | High |
| NFR-2 | Availability | 99.5% during close window (D-3 to D+2) | High |
| NFR-3 | Security & Compliance | Role-based access, SOX evidence retention 7 yrs | High |
| NFR-4 | Usability | Dashboard Net-Promoter-Score >40 from executives | Medium |
| NFR-5 | Scalability | Support 2× current volume without perf degradation | Medium |

## 10. Business Rules
* Invoices ≤ $50K do not require requester approval; system may auto-release BPA.
* Accrual reversals must be auto-reversing in the following period.
* COO approval required for requisitions > $100K.
* Labour purchases must use Oracle Blanket Purchase Agreements (BPAs).

## 11. Assumptions
* All SOWs are attached to the originating ServiceNow requisition.
* ServiceNow and Oracle APIs are available for integration.
* Data quality issues will be addressed in upstream systems within project timeline.

## 12. Dependencies
* ServiceNow API access & credentials.
* Oracle EBS R12 open-interface access for JE import and BPA release.
* Orbitz BI dataset availability.
* OneStream data-mart access for validation.

## 13. Constraints
* Project must not modify existing Oracle configurations beyond interface enablement.
* User adoption training limited to 1 day per stakeholder group.
* Initial phase excludes EchoStar/Hughes data until consolidation complete.

## 14. Risks & Issues
| Risk / Issue | Probability | Impact | Mitigation |
|---|---|---|---|
| Free-form requisition fields cause inaccurate accrual maths. | High | High | Implement form validation & AI fall-back heuristics. |
| Vendor non-compliance with invoice routing. | Medium | High | Automated reminders + AP enforcement. |
| Limited API throughput close-week. | Low | Medium | Schedule incremental loads; enable queue. |
| Security audit failure (SOX). | Low | High | Design audit logs & role segregation. |
...
* Integration with EchoStar/Hughes processes

## 16. Traceability Matrix
| Requirement ID | Source (Transcript timestamp) | Validation Method |
|---|---|---|
| FR-1 | Transcript 0:53-3:42 | "Unit test, UAT scenario, reconciliation report" |
| FR-2 | 9:22-11:35 | "Unit test, UAT scenario, reconciliation report" |
| FR-3 | 12:12-13:54 | "Unit test, UAT scenario, reconciliation report" |
| FR-4 | 22:07-23:40 | "Unit test, UAT scenario, reconciliation report" |
| FR-5 | 33:26-34:27 | "Unit test, UAT scenario, reconciliation report" |
| FR-6 | 34:27-36:22 | "Unit test, UAT scenario, reconciliation report" |
| FR-7 | 31:19-32:52 | "Unit test, UAT scenario, reconciliation report" |
| FR-8 | 45:42-46:33 | "Unit test, UAT scenario, reconciliation report" |
| FR-9 | 42:09-45:29 | "Unit test, UAT scenario, reconciliation report" |

## 17. Process Flow Descriptions
**17.1 Invoice Intake & Auto-Release (<= $50K)**
1. Vendor e-mails invoice → IT Expense inbox
2. AI Agent classifies e-mail & extracts PO number
3. Agent calls Oracle API: if no release exists and amount ≤ $50K, create BPA release
4. Oracle three-way match clears hold → invoice ready for payment

**17.2 Accrual Calculation & JE Posting**
1. Scheduler triggers agent at month-end cut-off
2. Agent pulls ServiceNow requisitions and Orbitz invoice data
3. For each requisition: compute accrual = (ceiling / term months) – invoices this period
4. Compile JE workbook, route for e-signature
5. Upon approval, auto-post via Oracle open-interface
6. Validate in OneStream and flag discrepancies

Visual swim-lane diagrams will be added once tooling is selected.

## 18. Approval & Sign-off
| Name | Role | Signature / Date |
|---|---|---|
| Trisha | TBD | ____________________ |
| Kartik Subbaraman | TBD | ____________________ |
| Payal Tiwana | TBD | ____________________ |

---
NOW, YOU WILL BE GIVEN A NEW TRANSCRIPT. USE THE INSTRUCTIONS AND THE EXAMPLE ABOVE TO GENERATE A NEW BRD OF THE SAME HIGH QUALITY. FOLLOW THIS GENERIC TEMPLATE FOR THE STRUCTURE:
...
## 6. Current State Overview
[Describe the current process as understood from the transcript, highlighting pain points and inefficiencies.]

## 7. Future State Vision
[Describe the envisioned future state with the proposed solution, focusing on improvements and benefits.]
...
// **NEW**: Prompt to intelligently extract a section from a document
...
const aiConfig = {
    ...
    openrouter: {
        ...
        brdGenerationModel: 'deepseek/deepseek-r1-0528:free',
        flowGenerationModel: 'deepseek/deepseek-r1-0528:free',
        sectionExtractionModel: 'deepseek/deepseek-r1-0528:free',
        apiBaseUrl: 'https://openrouter.ai/api/v1',
        siteUrl: 'http://localhost:3000', // Or your app's URL
        appName: 'Explora'
    },
    spacy: {
        pythonPath: 'python3',
...
// **NEW FUNCTION**: Sanitizes text to remove confusing formatting before sending to the AI.
const sanitizeTextForFlowchart = (text) => {
    if (!text) return '';
    const lines = text.split('\n');
    const sanitizedLines = lines
        // 1. Remove any lines that are purely visual separators (e.g., "---", "***")
        .filter(line => !/^[\s*\-=_]{3,}$/.test(line.trim()))
        // 2. Remove list markers (like "1.", "*", "-") from the start of lines but keep the text
        .map(line => line.replace(/^(\s*(\*|\-|\d+\.)\s*)+/, ''))
        // 3. Remove any resulting empty lines
...
app.post('/api/generate', upload.single('file'), async (req, res) => {
    const reqId = uuidv4().slice(0, 8);
    console.log(`[${reqId}] Received request for /api/generate`);
    if (!req.file) return res.status(400).json({ error: 'No file uploaded.' });

    let requestedArtifacts;
    try {
        requestedArtifacts = JSON.parse(req.body.artifacts);
        if (!Array.isArray(requestedArtifacts) || requestedArtifacts.length === 0) {
            return res.status(400).json({ error: 'No artifacts requested.' });
        }
    } catch (e) {
        return res.status(400).json({ error: 'Invalid artifacts format. Must be a JSON array.' });
    }

    console.log(`[${reqId}] User requested artifacts: ${requestedArtifacts.join(', ')}`);

    try {
        const generatedResults = {};
        
        // --- Step 1: Anonymization (Prerequisite for BRD) ---
        // This step is needed if BRD or any downstream artifact is requested.
        const needsAnonymization = requestedArtifacts.some(art => ['brd', 'anonymized', 'mapping', 'asisFlow', 'tobeFlow'].includes(art));
        let anonymizationData = {};
        if (needsAnonymization) {
            console.log(`[${reqId}] Performing Anonymization...`);
            const fileContent = await getFileContent(req.file);
            anonymizationData = await anonymizeText(fileContent);
        }

        if (requestedArtifacts.includes('anonymized')) {
            generatedResults.anonymized = {
                fileName: 'Anonymized_Input.txt',
                content: Buffer.from(anonymizationData.anonymizedText).toString('base64'),
                contentType: 'text/plain'
            };
        }
        if (requestedArtifacts.includes('mapping')) {
            let csvContent = "Code,Original_Entity\n";
            for (let [code, original] of anonymizationData.mapping.entries()) {
                csvContent += `${code},"${original.replace(/"/g, '""')}"\n`;
            }
            generatedResults.mapping = {
                fileName: 'Redaction_Key.csv',
                content: Buffer.from(csvContent).toString('base64'),
                contentType: 'text/csv'
            };
        }

        // --- Step 2: BRD Generation (Prerequisite for Flows) ---
        const needsBrd = requestedArtifacts.some(art => ['brd', 'asisFlow', 'tobeFlow'].includes(art));
        let brdText = '';
        if (needsBrd) {
            console.log(`[${reqId}] Generating BRD...`);
            brdText = await generateBRD(anonymizationData.anonymizedText);
        }

        if (requestedArtifacts.includes('brd')) {
            let finalBRDText = brdText;
            for (let [code, original] of anonymizationData.mapping.entries()) {
                finalBRDText = finalBRDText.replace(new RegExp(`\\b${code}(?![a-zA-Z0-9_])`, 'g'), original);
            }
            const docxBuffer = await createDocxBufferFromMarkdown(finalBRDText);
            generatedResults.brd = {
                fileName: 'BRD_Explora.docx',
        ...
        if (requestedArtifacts.includes('asisFlow')) {
            ...
            generatedResults.asisFlow = {
                fileName: 'As_Is_Flow.txt',
                content: Buffer.from(mermaidCode).toString('base64'),
                contentType: 'text/plain'
            };
        }
        if (requestedArtifacts.includes('tobeFlow')) {
            ...
            generatedResults.tobeFlow = {
                fileName: 'To_Be_Flow.txt',
                content: Buffer.from(mermaidCode).toString('base64'),
                contentType: 'text/plain'
            };
        }
